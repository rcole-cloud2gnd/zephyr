...
static uint8_t const peripheral_adv_static_rand[] = {0xD0, 0xD1, 0xD2, 0xD3, 0xD4, 0xD5};
static uint8_t const central_adv_static_rand[]    = {0xB0, 0xB1, 0xB2, 0xB3, 0xB4, 0xB5};
...
static bool volatile periph_is_scanned, periph_is_connected, periph_is_disconnected;
static bool volatile central_is_scanned, central_is_connected, central_is_disconnected;
...
static void periph_connected(struct bt_conn *conn, uint8_t conn_err)
{
	...
	printk("Connected (Peripheral).\n");

	periph_is_connected = true;
#if 0
...
{
	printk("Disconnected (Peripheral).\n");

	periph_is_disconnected = true;
}

BT_CONN_CB_DEFINE(periph_conn_callbacks) = {
...
struct bt_le_ext_adv_cb periph_adv_callbacks = {
	.sent = sent_cb,
	.connected = connected_cb,
	.scanned = scanned_cb,
};

struct bt_le_ext_adv_cb central_adv_callbacks = {
	.sent = sent_cb,
	.connected = connected_cb,
	.scanned = scanned_cb,
};
...
static void test_adv_resume_main(void)
{
	...
	printk("\n*Advertising with NRPA Resume Test (Advertiser)*\n");
	...
	// @todo COnfigure NRPA for advertising

	printk("Configuring static random address\n");

#if 0
	// The ext adv record is needed to set the static random address, but the record is not
	// accessible unless advertising is started if extended advertisements are not supported. 
	// This makes it impossible to set the static random address for legacy advertising 
	// before using it. bt_le_adv_lookup_legacy() fails at this point.
	ext_adv = bt_le_adv_lookup_legacy();
	if (!ext_adv) {
		printk("Unable to acquire ext_adv record\n");
		goto exit;
	}
#endif
	...
	err = bt_le_ext_adv_create(BT_LE_EXT_ADV_CONN_NAME, &periph_adv_callbacks, &adv);
	if (err) {
		printk("Failed to create extended advertsiing set (connectable) (err %d)\n", err);
		goto exit;
	}
	printk("success.\n");

	le_addr[0] = '\x0';
	bt_addr_le_to_str((struct bt_addr_le_t *)peripheral_adv_static_rand, le_addr, sizeof(le_addr));

	printk("Setting random address to %s ", le_addr);
	err = bt_id_set_adv_random_addr(adv, &peripheral_adv_static_rand);
	if (err) {
		printk("Failed to set random address (err %d)\n", err);
		goto exit;
	}
	printk("success.\n");
...
#if defined(CONFIG_BT_PRIVACY) && defined(CONFIG_BT_RPA_TIMEOUT)
	ext_adv_param.timeout = MIN(800 * 10, CONFIG_BT_RPA_TIMEOUT - 1);
#else	
	ext_adv_param.timeout = 800 * 10;
#endif	
...
static void test_scan_resume_main(void)
{
	struct bt_le_ext_adv_start_param ext_adv_param;
	struct bt_le_ext_adv *adv;
	char le_addr[BT_ADDR_LE_STR_LEN];
	...
	printk("\n*Advertising with NRPA Resume Test (Scanner)*\n");
	...
	// Start advertising in the Central role as well
#if defined(CONFIG_BT_EXT_ADV)
	printk("Create connectable extended advertising set...");
	err = bt_le_ext_adv_create(BT_LE_EXT_ADV_CONN_NAME, &central_adv_callbacks, &adv);
	if (err) {
		printk("Failed to create extended advertsiing set (connectable) (err %d)\n", err);
		goto exit;
	}
	...
	le_addr[0] = '\x0';
	bt_addr_le_to_str((struct bt_addr_le_t *)central_adv_static_rand, le_addr, sizeof(le_addr));

	printk("Setting random address to %s ", le_addr);
	err = bt_id_set_adv_random_addr(adv, &central_adv_static_rand);
	if (err) {
		printk("Failed to set random address (err %d)\n", err);
		goto exit;
	}
	...
	printk("Waiting for connection...");
	while (!central_is_connected) {
		k_sleep(K_MSEC(100));
	}
	printk("success.\n");

	printk("Waiting for disconnect...");
	while (!central_is_disconnected) {
		k_sleep(K_MSEC(100));
	}
	printk("success.\n");
...
static void test_adv_tick(bs_time_t HW_device_time)
{
	bst_result = Failed;
...
static const struct bst_test_instance test_def[] = {
	{
		...
		.test_post_init_f = test_adv_init,
		.test_tick_f = test_adv_tick,
		.test_main_f = test_adv_resume_main
	},
	{
		.test_id = "scan_resume",
		...
		.test_post_init_f = test_adv_init,
		.test_tick_f = test_adv_tick,
		.test_main_f = test_scan_resume_main
	},
	BSTEST_END_MARKER
};
